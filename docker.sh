...
# args+=" -v $(pwd)/startup.liz:/root/rosys/catkin_ws/src/robot/startup.liz"
args+=" -v $(pwd)/:/root/home/ros2_workspace/"
args+=" -v $esp:/dev/esp"
args+=" --device=$esp"
args+=" -p 80:80"
...
do
    case $cmd in
        b | build)
            docker build --network host --tag $name . || exit ;;
        r | run)
            docker run --network host -it $args $name || exit ;;
        s | shell)                     
